  ...
  actions: {
    async getPolygons(idEvent: number) {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/poligonos/evento/${idEvent}/public`
      );
    ...
    async getOnePolygon(id: number) {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/poligonos/${id}/public`
      );
    ...
    async createPolygon(polygon: PolygonCreate) {
      const response = await axios
        .post(
          `${import.meta.env.VITE_API_URL}/poligonos/evento/${
            polygon.eventoId
          }`,
          polygon.locais,
          {
    ...
    async updatePolygon(polygon: Polygon) {
      const response = await axios
        .patch(
          `${import.meta.env.VITE_API_URL}/poligonos/${polygon.id}`,
          polygon,
          {
            headers: {
              Authorization: `Bearer ${this.token}`,
    ...
    async deletePolygon(id: number) {
      const response = await axios
        .delete(`${import.meta.env.VITE_API_URL}/poligonos/${id}`, {
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
        })
